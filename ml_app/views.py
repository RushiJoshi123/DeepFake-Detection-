...
import cv2
import matplotlib
matplotlib.use('Agg')  # <<< added (for servers without GUI)
...
def predict(model,img,path = './', video_file_name=""):
  ...
  return [int(prediction.item()),confidence, logits]   # <<< return logits too
...
def plot_heat_map(i, model, img, path = './', video_file_name=''):
  ...
  return heatmap_name  # Return just the filename, not the full path
    ...
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            if settings.DEBUG:
                ...
                video_path = os.path.join(settings.PROJECT_DIR, 'uploaded_videos','app','uploaded_videos', saved_video_file)
                with open(video_path, 'wb') as vFile:
                    shutil.copyfileobj(video_file, vFile)

            # Process the video immediately and show results on index page
            try:
                results = process_video(video_path, sequence_length)
                results['form'] = video_upload_form
                results['video_processed'] = True
                print(f"DEBUG: Results keys: {results.keys()}")
                print(f"DEBUG: Histogram file: {results.get('confidence_histogram')}")
                print(f"DEBUG: Line graph file: {results.get('confidence_line_graph')}")
                return render(request, index_template_name, results)
            except Exception as e:
                print(f"Exception occurred during prediction: {e}")
                import traceback
                traceback.print_exc()
                video_upload_form.add_error(None, f"Error processing video: {str(e)}")
                return render(request, index_template_name, {"form": video_upload_form})
        else:
            return render(request, index_template_name, {"form": video_upload_form})
...
def process_video(video_file, sequence_length):
    """Process video and return context with results including histogram"""
    ...
    path_to_model = model_filename  # get_accurate_model already returns full path
    ...
    for i in range(len(path_to_videos)):
        ...
        # collect frame-wise confidences
        logits = prediction[2]
        for j in range(sequence_length):
            frame_conf = logits[:, prediction[0]].item() * 100
            frame_confidences.append(frame_conf)
            heatmap_images.append(
                plot_heat_map(j, model, video_dataset[i], './', video_file_name_only)
            )

    # --- Plot confidence graphs ---
    graph_folder = os.path.join(settings.PROJECT_DIR, 'graph')
    os.makedirs(graph_folder, exist_ok=True)
    
    # Line graph for frame-wise confidence
    line_graph_path = os.path.join(graph_folder, f"{video_file_name_only}_confidence_line.png")
    plt.figure(figsize=(12, 6))
    plt.plot(range(1, len(frame_confidences)+1), frame_confidences, marker='o', color='b', linewidth=2, markersize=4)
    plt.title("Frame-wise Confidence Score", fontsize=14, fontweight='bold')
    plt.xlabel("Frame Number", fontsize=12)
    plt.ylabel("Confidence (%)", fontsize=12)
    plt.ylim(0, 100)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(line_graph_path, dpi=150, bbox_inches='tight')
    plt.close()
    
    # Histogram for confidence distribution
    histogram_path = os.path.join(graph_folder, f"{video_file_name_only}_confidence_histogram.png")
    plt.figure(figsize=(10, 6))
    
    # Create histogram with appropriate bins
    n_bins = min(20, len(set(frame_confidences)))  # Adaptive number of bins
    plt.hist(frame_confidences, bins=n_bins, alpha=0.7, color='skyblue', edgecolor='black', linewidth=0.5)
    
    # Add statistics
    mean_conf = np.mean(frame_confidences)
    std_conf = np.std(frame_confidences)
    plt.axvline(mean_conf, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_conf:.1f}%')
    plt.axvline(mean_conf + std_conf, color='orange', linestyle=':', linewidth=1.5, label=f'+1 STD: {mean_conf + std_conf:.1f}%')
    plt.axvline(mean_conf - std_conf, color='orange', linestyle=':', linewidth=1.5, label=f'-1 STD: {mean_conf - std_conf:.1f}%')
    
    plt.title("Confidence Score Distribution (Histogram)", fontsize=14, fontweight='bold')
    ...
    return {
        ...
        'confidence_line_graph': os.path.basename(line_graph_path),
        'confidence_histogram': os.path.basename(histogram_path),
        'original_video': production_video_name,
        'output': output,
        'confidence': confidence,
        'frame_confidences': frame_confidences,
        'mean_confidence': round(np.mean(frame_confidences), 1),
        'std_confidence': round(np.std(frame_confidences), 1)
    }
    ...
    if request.method == "GET":
        ...
        try:
            ...
            context = {
                ...
                'output': output,
                'confidence': confidence,
                'frame_confidences': frame_confidences,  # Pass raw data for potential JS usage
                'mean_confidence': round(np.mean(frame_confidences), 1),
                'std_confidence': round(np.std(frame_confidences), 1)
            }
